...
public class DBDataLoader implements DataLoader {
	...
	public void doReadData(CacheSet cacheSet) {
		...
		int NuOfKeys = InMemoryProperties.getInstance()
				.getIntPropertyValue("InMemory." + cacheSet.getCacheId() + ".dataloader.nu_of_keys");

		String sql = InMemoryProperties.getInstance()
				.getPropertyValue("InMemory." + cacheSet.getCacheId() + ".dataloader.sql");

		List<String[]> hybernateList = InMemoryProperties.getInstance()
				.getPropList("InMemory." + cacheSet.getCacheId() + ".dataloader.hibernate");

		Configuration cfg = new Configuration();

		for (String[] propArr : hybernateList) {
			cfg.setProperty(
					propArr[0].replace("InMemory." + cacheSet.getCacheId() + ".dataloader.hibernate", "hibernate"),
					propArr[1]);
		}
